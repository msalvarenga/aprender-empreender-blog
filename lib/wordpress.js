const WP_API = process.env.NEXT_PUBLIC_WP_API_URL || 'https://aprenderempreender.com.br/wp-json/wp/v2'
...
export function getAuthorName(post) {
  return post?._embedded?.author?.[0]?.name || 'Prof. M. Alvarenga'
}
...
export function getCategoryColor(post) {
  const slug = getCategorySlug(post)
  const map = {
    'educacao': 'brand',
    'matematica': 'brand',
    'empreendedorismo': 'gold',
    'enem-vestibular': 'brand',
    'raciocinio-logico': 'gold',
    'para-professores': 'brand',
    'metodologias': 'brand',
  }
  return map[slug] || 'brand'
}
